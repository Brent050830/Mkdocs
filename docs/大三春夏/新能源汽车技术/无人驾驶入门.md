...
**为什么规划时拿 `loop_route.last_index`，不能直接拿 `_tracking_route` 的 index？**

核心原因是：**replacement segment 的 start_s / end_s 是定义在基础路线坐标系里的，不是定义在 `_tracking_route` 的变形路线坐标系里的。**

也就是说，写入 replacement 的时候用的是：
...
因为 `_tracking_route` 已经被 replacement 改过了，它的点列和弧长可能和基础路线不完全一致。尤其是避障段弯出去以后：
...
如果直接用 `_tracking_route` 上的 index/s 去写 replacement，就可能出现：

```text
你以为替换的是基础路线 [100m, 130m]
实际却把合成路线上的 [100m, 130m] 当成基础路线区间写回去
```
...
replacement 写入 / 候选轨迹定义：
    用 base route 坐标，也就是 loop_route / _base_tracking_route 的 s
```

---

**那之前 MPC 转角突变是哪条路线引起的？**

不是 MPC 本身突然想打方向，而是 **回归段新写入后，当前合成路线 `_tracking_route` 在车辆附近发生了几何不连续或进度不连续**。
...
但车辆当时已经在避障后的 `_tracking_route` 上行驶。

这时存在两个进度：

```text
s_base    = 车辆投影到基础路线 C_base 上的真实进度
s_active  = 车辆投影到当前合成路线 C_track 上的进度
```

之前代码用 `loop_route.last_index` 推出来的 `start_route_s`，可能和车辆真实所在的 `s_base` 有偏差。
...
然后 `_tracking_route` 被重新合成后，MPC 下一帧看到的参考线突然变了，于是第一拍可能给出很大的转角。

所以根因路线是：

```text
新合成出来的 _tracking_route 在回归段起点附近不连续
```

而造成这个不连续的输入原因是：

```text
回归段 start_route_s 用了 loop_route.last_index 推算，
没有用车辆当前位置真实投影到 base route 的 s_start/d_start
```
...
也就是回归段从车辆当前真实基础路线投影处开始生成，而不是单纯依赖 `loop_route.last_index`。
