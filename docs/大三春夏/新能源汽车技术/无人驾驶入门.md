...
- 先把候选筛选原因打印出来：比如 valid=1/12, rejected={'lateral acceleration too high': 7, 'path too long before front vehicle': 4}，这样确认到底为什么只剩一条。  
- 弯道上不要允许太短避障轨迹：如果当前路线曲率/转向事件较大，最低长度从 14m 提高到比如 22m；如果距离不够，就先跟车减速，不要硬生成短急避障。  
- 用平滑后的路线切线计算右向量，而不是直接拿每个 waypoint 的右向量；也就是根据前后路线点差分算 tangent，再算 right，这样偏移轨迹会连续很多。  
- 需要的话把 debug 轨迹绘制步长从 2.0m 改成 1.0m，画面会更细，但这只是视觉改善，不是根本算法改善。

- 能不能都变成轨迹坐标的计算（状态切换、轨迹规划等等）
- 动态的障碍还是解决不了
- MPC 跟踪时的速度问题没有解决
- 轨迹的平滑没有解决
- 还有就是坐标的变换没有解决
