...
| 状态                 | 进入条件                                                     | 退出条件                                                     |
| -------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| `FOLLOW`             | 初始状态。                                                   | 若 $\mathrm{emergency\_needed}$ 且 $\mathrm{avoidance\_side}\ne\mathrm{None}$，进入 `AVOID`；若 $\mathrm{emergency\_needed}$ 且 $\mathrm{avoidance\_side}=\mathrm{None}$，进入 `EMERGENCY_BRAKE`；若 $\mathrm{right\_object\_risk}$ 且 $\neg\mathrm{right\_object\_yield\_done}$，进入 `RIGHT_OBJECT_YIELD`；若已完成路线并记录 `route_completion_time`，进入 `ROUTE_HOLD`。 |
| `AVOID`              | 当前状态为 `FOLLOW` 或 `LANE_KEEP`，且 $\mathrm{emergency\_needed}$ 成立，并且相邻车道存在可用避障方向。进入时基于当前自车位姿重新生成 `QuinticLaneChangeTrajectory`。 | 若换道进度满足 $s_{\mathrm{traj}} > L_{\mathrm{lanechange}} + 8.0$ 且 $|d_{\mathrm{traj}} - D| < 0.65$，进入 `LANE_KEEP`；若 `route_completion_time` 已记录，进入 `ROUTE_HOLD`。 |
| `LANE_KEEP`          | `AVOID` 完成后进入；或 `RIGHT_OBJECT_YIELD` 风险解除后进入。 | 若 $\mathrm{emergency\_needed}$ 且 $\mathrm{avoidance\_side}\ne\mathrm{None}$，再次进入 `AVOID` 并生成新的换道轨迹；若 $\mathrm{emergency\_needed}$ 且 $\mathrm{avoidance\_side}=\mathrm{None}$，进入 `EMERGENCY_BRAKE`；若 $\mathrm{right\_object\_risk}$ 且 $\neg\mathrm{right\_object\_yield\_done}$，进入 `RIGHT_OBJECT_YIELD`；若 `route_completion_time` 已记录，进入 `ROUTE_HOLD`。 |
| `EMERGENCY_BRAKE`    | 当前状态为 `FOLLOW` 或 `LANE_KEEP`，且 $\mathrm{emergency\_needed}$ 成立，但左右邻道均不可用。 | 当前代码没有恢复退出条件，会持续全制动；只有路线完成覆盖到 `ROUTE_HOLD`、碰撞/窗口关闭/仿真时间结束等外部终止条件会结束该状态。 |
| `RIGHT_OBJECT_YIELD` | 当前状态为 `FOLLOW` 或 `LANE_KEEP`，且 $\mathrm{right\_object\_risk}$ 成立，并且 `right_object_yield_done == False`。 | 若 $\neg\mathrm{right\_object\_risk}$，进入 `LANE_KEEP`，并设置 `right_object_yield_done = True`；若 `route_completion_time` 已记录，进入 `ROUTE_HOLD`。 |
