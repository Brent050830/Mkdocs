...
\begin{cases}
T_{\max}, & n \le n_b\\
\min\left(T_{\max}, \frac{P_{\max}}{\omega_m}\right), & n>n_b
\end{cases}
$$
...
T_{\regen,\max}(n)=
\begin{cases}
T_{\regen,\max}, & n \le n_b\\
\min\left(T_{\regen,\max}, \frac{P_{\regen,\max}}{\omega_m}\right), & n>n_b
\end{cases}
$$
...
1. 统一参考轨迹和智能体的动力学口径  
文件: [motor_env.py](c:/Users/17871/Desktop/节能/motor_env.py)  
现在参考轨迹还是“先定速度，再反推扭矩”，智能体却已经用新 plant 在跑。要改成:
- 参考驾驶员先输出“目标扭矩/目标加速度”
- 再调用同一个 [step_longitudinal_dynamics()](c:/Users/17871/Desktop/节能/motor_env.py#L219) 滚出 `ref_speed/ref_torque/ref_energy`
- 参考和智能体都走同一套限功率、SOC、电池功率、传动效率
...
2. 把奖励从硬门控改成连续门控  
文件: [motor_env.py](c:/Users/17871/Desktop/节能/motor_env.py#L631)  
...
3. 改 best checkpoint 选择规则  
文件: [motor_training.py](c:/Users/17871/Desktop/节能/motor_training.py#L285)  
现在更像在选“别太极端”的模型，不是在选“最省电”的模型。建议改成:
- 先筛 `tracking_ok=True`
- 再筛 `bias_guard_ok=True`
- 在剩下模型里按 `saving_total_pct` 最大选
- 作为辅条件要求 `saving_cmp_total_pct > 0`
...
4. 重做动作空间  
文件: [motor_env.py](c:/Users/17871/Desktop/节能/motor_env.py#L477)  
现在动作只是参考扭矩的小残差，太容易“修坏参考”。建议二选一:
- 保守版: 保留残差，但做课程学习，逐步放大动作范围
- 更稳版: 动作改成“目标轮端力修正”或“目标加速度修正”

5. 奖励分成“局部可学”和“最终目标”两层  
文件: [motor_env.py](c:/Users/17871/Desktop/节能/motor_env.py#L681) [motor_env.py](c:/Users/17871/Desktop/节能/motor_env.py#L700)  
建议:
- 单步奖励继续用 `loss_energy`
- 终局奖励改成 `battery energy + 时间偏差 + 跟踪误差`  
这样局部稳定、全局目标也不丢。
...
6. 评估时固定输出 4 类诊断  
文件: [motor_env.py](c:/Users/17871/Desktop/节能/motor_env.py#L767) [motor_plotting.py](c:/Users/17871/Desktop/节能/motor_plotting.py)
- 牵引段 / 回收段分开统计
- 按速度区间统计节能率
- 按扭矩区间统计节能率
- 画 ref/agent 在效率图上的落点热图
...
### 动作的更新
每一距离步里，速度、位移和能耗的更新顺序现在是这样的，入口在 [motor_env.py:474](c:/Users/17871/Desktop/节能/motor_env.py#L474)：
1. 先把动作变成残差扭矩，再和参考扭矩合成当前步的扭矩命令，在 [motor_env.py:477](c:/Users/17871/Desktop/节能/motor_env.py#L477) 到 [motor_env.py:483](c:/Users/17871/Desktop/节能/motor_env.py#L483)。
2. 把这个扭矩命令送进同一个动力学步进器 [motor_env.py:485](c:/Users/17871/Desktop/节能/motor_env.py#L485)。
3. 在步进器里，先用当前车速算电机转速，再做恒扭矩/恒功率限幅，在 [motor_env.py:228](c:/Users/17871/Desktop/节能/motor_env.py#L228) 到 [motor_env.py:229](c:/Users/17871/Desktop/节能/motor_env.py#L229)。
4. 然后算滚阻、坡阻、风阻、驱动力和合力，再得到加速度，在 [motor_env.py:231](c:/Users/17871/Desktop/节能/motor_env.py#L231) 到 [motor_env.py:237](c:/Users/17871/Desktop/节能/motor_env.py#L237)。
5. 再按距离域公式更新速度：  
\[  
v_{k+1}=\sqrt{\max(v_k^2+2a_k\Delta s,\ 0.25)}  
\]  
见 [motor_env.py:238](c:/Users/17871/Desktop/节能/motor_env.py#L238)。
6. 用新旧速度平均值得到本步时间：  
\[  
v_{\text{avg},k}=\frac{v_k+v_{k+1}}{2},\qquad \Delta t_k=\frac{\Delta s}{v_{\text{avg},k}}  
\]  
见 [motor_env.py:240](c:/Users/17871/Desktop/节能/motor_env.py#L240) 到 [motor_env.py:241](c:/Users/17871/Desktop/节能/motor_env.py#L241)。
7. 用平均转速查效率图，算电池功率和单步电能，在 [motor_env.py:243](c:/Users/17871/Desktop/节能/motor_env.py#L243) 到 [motor_env.py:248](c:/Users/17871/Desktop/节能/motor_env.py#L248)。
8. 同时算奖励口径里的损失能量 `cmp_energy_step`，在 [motor_env.py:249](c:/Users/17871/Desktop/节能/motor_env.py#L249)。
9. 最后回到环境里累计：  
\[  
...
\]  
对应 [motor_env.py:513](c:/Users/17871/Desktop/节能/motor_env.py#L513) 到 [motor_env.py:518](c:/Users/17871/Desktop/节能/motor_env.py#L518)。
...
> 	第二步（后算能耗）： 功率只是个瞬间强度。用这个平均功率 $\times$ 第 4 步算出的时间 $\Delta t_k$ （公式原理是 $E = P \times \Delta t$ ），最终得到了走完这段 $\Delta s$ 一共消耗掉的电能（代码里的energy_step）。
> 	
> 6. 更新并进入新循环
> 把算出来的末端速度 $v_{k+1}$ 记作下一段路程的“初始速度”，往前挪动 $\Delta s$ 的距离。然后把这个最新状态返回给智能体，智能体根据新状态再次给出下一段 $\Delta s$ 的新扭矩指令，无限重复。
